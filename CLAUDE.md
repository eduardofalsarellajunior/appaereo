...
- Schema SQL escrito em `supabase/sql/001_schema.sql` (6 tabelas: `intencoes_viagem`,
...
- Seed de teste em `supabase/sql/002_seed_fase1.sql`: 1 intenção de teste
  ("BR -> Japão (teste)") + 1 rota candidata **fixa e manual** (GRU → NRT), usada só
  para validar a integração Duffel antes do motor de geração de rotas existir.
- Integração Duffel implementada em duas camadas:
  - `supabase/functions/buscar-tarifa-fase1/index.ts` — Edge Function que busca a
    tarifa da rota fixa e grava em `price_history`. É o único ponto que efetivamente
    chama o Duffel e escreve no banco.
...
- Dashboard completo (Fase 5) ainda não existe; há só `src/pages/TesteFase1.jsx`, uma
  tela mínima para disparar a busca e ver o histórico de preço da rota fixa.

### Pendências conhecidas

- O projeto Supabase de destino ainda precisa ser criado/confirmado pelo usuário (a
  conta free permite só 2 projetos; este projeto ficará em outra conta/projeto
  dedicado). O schema SQL só deve ser executado depois que a connection string
  correta for confirmada.
- Deploy da Edge Function, `supabase secrets set` (DUFFEL_API_KEY,
  SUPABASE_SERVICE_ROLE_KEY) e variáveis de ambiente na Vercel são passos manuais do
  usuário — não executáveis a partir do ambiente onde este projeto foi montado (sem
  Supabase CLI autenticado nem token da Vercel).

## Roadmap

| Fase | Entrega | Status |
|---|---|---|
| 1 | Schema Supabase + integração Duffel (cash apenas) para 1 rota de teste fixa | Em andamento |
